...
class CalibrateDragNewActionServer(Node):
    ...
    def __init__(self):
        ...
        self.force_pub = self.create_publisher(
            Twist, "controller/active_body_force", qos_profile_system_default)

        # TODO: make trigger topic
        self.trigger_sub = self.create_subscription(
            Empty, "trigger", self.trigger_cb, qos_profile_system_default, callback_group=ReentrantCallbackGroup())

        self.odometry_sub = self.create_subscription(
            Odometry, "odometry/filtered", self.odometry_cb, qos_profile_system_default, callback_group=ReentrantCallbackGroup())
        ...
        if not os.path.exists(self.csvPath):
            self.get_logger().info(
                f"Proposed csv path {self.csvPath} does not exist; falling back to home directory")
    ...
    def collect_data(self, axis):
        force_data = [-49, -42, -35, -28, -21, -14, -
                      7] if (axis < 2) else [-21, -18, -15, -12, -9, -6, -3]
    ...
    def run_until_stable(self, force, axis):
        self.get_logger().info(
            f"Running until stable with force {force} on axis {axis}")
