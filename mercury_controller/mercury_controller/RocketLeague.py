...
class RocketLeague(Node):
    ...
    def __init__(self, queue: Queue, typeQueue: Queue):
        ...
        self.throttle_pub = self.create_publisher(
            Float32, "/talos/teleop/throttle", qos_profile=qos_profile_sensor_data)
        self.reverse_pub = self.create_publisher(
            Float32, "/talos/teleop/reverse", qos_profile=qos_profile_sensor_data)
        self.joy_L_X_pub = self.create_publisher(
            Float32, "/talos/teleop/joy_R_X", qos_profile=qos_profile_sensor_data)
        self.set_point_pub_linear = self.create_publisher(
            ControllerCommand, "/talos/controller/linear", qos_profile=qos_profile_system_default)
        self.set_point_pub_angular = self.create_publisher(
            ControllerCommand, "/talos/controller/angular", qos_profile=qos_profile_system_default)
        self.apply_nudge_pub = self.create_publisher(
            Twist, "/talos/teleop/apply_nudges", qos_profile=qos_profile_sensor_data)

        # action pubs
        self.boost_trigger_pub = self.create_publisher(
            Bool, "/talos/teleop/trigger_boost", qos_profile=qos_profile_system_default)
        self.stunt_trigger_pub = self.create_publisher(
            UInt16, "/talos/controller/stunt_state", qos_profile=qos_profile_system_default)
        self.torpedo_trigger_pub = self.create_publisher(
            Empty, "/talos/command/simple_torpedo_fire", qos_profile_system_default)
        self.dropper_trigger_pub = self.create_publisher(
            Empty, "/talos/command/simple_dropper_fire", qos_profile_system_default)
    ...
    def handleEvents(self):
                ...
                if (event.code == mappings[self.controller_type]["a_btn"]):
                    ...
                    if (event.value == 1):
                        if (self.stunt_triggered == False):
                            ...
                            self.stunt_cancel_timer = self.create_timer(
                                1, self.detriggerStunt)
                    ...
                    if (event.value == 1):
                        if (not self.boost_triggered):
                            ...
                            self.boost_cancel_timer = self.create_timer(
                                5, self.detriggerBoost)
        ...
        qaut = quaternion_from_euler(
            self.left_joy_y * self.pitch_max, self.left_joy_x * self.roll_max, 0, 'sxyz')
...
async def read_controller_events():
    ...
    while True:
        ...
        if controller is None:
            ...
            # name of the controller trying to find
            controllerTypes = ["PLAYSTATION(R)3", "SHANWAN"]
