...
class MonitoredServiceClient():
    def __init__(self, node: Node, srv_type: type, srv_name: str):
        ...
        # if active, should be a tuple of (request, callback)
    ...
    def schedule_call(self, request, callback):
        ...
        self.node.get_logger().error(
            f"Reached limit of calls for service {self.client.srv_name}")
        return False
    ...
    def _timer_callback(self):
        ...
        if self.waiting_for_client:
            ...
            if (current_time - self.srv_start_time).to_msg().sec >= 3:
                self.node.get_logger().error(
                    f"Call to service {self.client.srv_name} timed out.")
            ...
            if len(self.waiting_requests) > 0:
                ...
                self.node.get_logger().debug(
                    f"Making call to service {self.client.srv_name}")
    ...
    def _srv_callback(self, future: rclpy.Future):
        ...
        if self.active_client is not None:
            ...
            self.node.get_logger().error(
                "MonitoredServiceClient received a callback but doesn't know where it came from")
...
class SimulinkModelNode():
    def __init__(self, node: 'ControllerOverseer', node_name: str):
        ...
        self.known_params = []
        self.reload_param_service = node.create_service(
            Trigger, f"controller_overseer/update_{self.node_name}_params".lower(), self.reload_parameters_callback)
    ...
    def check_if_active(self, active_nodes: 'list[str]'):
        active_model_nodes = [
            node for node in active_nodes if self.node_name in node]
        num_active_model_nodes = len(active_model_nodes)

        if num_active_model_nodes > 1:
            # not useful, but interesting. Print it out
            self.overseer_node.get_logger().warning(
                f"Detected {num_active_model_nodes} active nodes with the name {self.node_name}!")
        ...
        if num_active_model_nodes == 1:
            ...
            if not self.model_active:
                ...
                self.overseer_node.get_logger().info(
                    f"Found {self.node_name} as {self.full_node_name}!")

                # the client to set the params of the model
                self.list_param_client = MonitoredServiceClient(
                    self.overseer_node, ListParameters, f"{self.full_node_name}/list_parameters")
                self.set_param_client = MonitoredServiceClient(
                    self.overseer_node, SetParameters, f"{self.full_node_name}/set_parameters")
    ...
    def list_and_set_model_parameters(self):
        ...
        self.overseer_node.get_logger().warning(
            f"Cannot list parameters for {self.node_name} because the model is not active or the list parameters client is None")
        return False
            ...
            for parameter_name in parameters:
                ...
                if not read_value is None:
                    if type == int:
                        ...
                        self.overseer_node.get_logger().warn(
                            f"Parameter type {type} not handled yet")
                    ...
                    self.overseer_node.get_logger().info(
                        f"Setting {parameter_name} to {read_value}")
                else:
                    self.overseer_node.get_logger().warning(
                        f"Not setting param {parameter_name}, no param found in config file!")
                    self.known_params.append(parameter_name)

            rq.parameters = param_array
            self.set_param_client.schedule_call(
                rq, self.set_parameters_done_callback)
        else:
            self.overseer_node.get_logger().warning(
                f"Cannot list parameters for {self.node_name} because the model is not active or the set parameters client is None")
    ...
    def set_model_parameters_from_list_callback(self, future: rclpy.Future, request):
        ...
        unknown_param_names = [
            name for name in param_names if name not in self.known_params]
    ...
    def set_parameters_done_callback(self, future: rclpy.Future, request):
        ...
        fail_indices = [idx for idx in range(
            0, len(result.results)) if not result.results[idx].successful]
        ...
        if len(fail_indices) == 0:
            self.overseer_node.get_logger().info(
                f"Successfully set parameters for {self.node_name}")
        else:
            for fail in fail_indices:
                self.overseer_node.get_logger().error(
                    f"Failed to set parameter {request.parameters[fail].name} for {self.node_name}: {result.results[fail].reason}")
...
class ControllerOverseer(Node):
    ...
    def __init__(self):
        ...
        self.thrusterSolverName = self.get_parameter(
            "thruster_solver_node_name").value
        ...
        self.submergedThrusters = [True, True,
        ...
        # thruster telemetry
        self.create_subscription(DshotPartialTelemetry, "state/thrusters/telemetry",
                                 self.thrusterTelemetryCB, qos_profile_system_default)

        # publish to active controllers whether motion is enabled
        self.motionEnabledPub = self.create_publisher(
            Bool, "controller/motion_enabled", qos_profile=qos_profile_system_default)

        # thruster solver parameters
        self.setThrusterSolverParamsClient = self.create_client(
            SetParameters, f"{self.thrusterSolverName}/set_parameters")

        # thruster mode
        self.create_subscription(Int16, "thrusterSolver/thrusterState",
                                 self.setThrusterModeCB, qos_profile_system_default)

        # odometry filtered
        self.create_subscription(
            Odometry, "odometry/filtered", self.odometryCB, qos_profile_system_default)

        # sub to autotune
        self.create_subscription(
            Twist, "ff_auto_tune", self.ffAutoTuneCB, qos_profile_system_default)

        # sub to the drag compensator forward
        self.create_subscription(TwistWithCovariance, "controller/drag_comp/forward",
                                 self.dragCompForwardCb, qos_profile_system_default)

        # sub to the drag compensator reverse
        self.create_subscription(TwistWithCovariance, "controller/drag_comp/reverse",
                                 self.dragCompRevereseCb, qos_profile_system_default)

        # pub for thruster weights
        self.weightsPub = self.create_publisher(
            Int32MultiArray, THRUSTER_SOLVER_WEIGHT_MATRIX_TOPIC, qos_profile_system_default)

        # pub ff force
        self.ffPub = self.create_publisher(
            Twist, FF_TOPIC_NAME, qos_profile_system_default)

        # pub signal to reinit autoff and dragcal
        self.re_init_signal_pub = self.create_publisher(
            Empty, AUTOTUNE_REINIT_TOPIC_NAME, qos_profile_system_default)

        # create teleop serivce
        self.setTeleop = self.create_service(
            SetBool, "setTeleop", self.setTeleop)
        ...
        self.create_timer(WEIGHTS_FORCE_UPDATE_PERIOD,
        ...
        self.escPowerCheckTimer = self.create_timer(
            ESC_POWER_TIMEOUT, self.escPowerTimeout)
    ...
    def generateThrusterForceMatrix(self, thruster_info, com):
        ...
        for i, thruster in enumerate(thruster_info):
            ...
            forceVector = np.matmul(euler_matrix(pose[3], pose[4], pose[5])[
                                    :3, :3], np.array([1, 0, 0]))
            positionFromCOM = pose[:3] - com
            torque = np.cross(positionFromCOM[:3], forceVector)

            # insert into thruster effect matrix
            self.thrusterEffects[i] = [
                forceVector[0], forceVector[1], forceVector[2], torque[0], torque[1], torque[2]]
    ...
    def setConfigPath(self):
        ...
        if self.configPath == '':
            ...
            descriptions_share_dir = get_package_share_directory(
                "riptide_descriptions2")
            robot_config_subpath = os.path.join(
                "config", self.robotName + ".yaml")

            # Set fallback config path if we can't find the one in source
            self.configPath = os.path.join(
                descriptions_share_dir, robot_config_subpath)
            ...
            if 'install' in dir_split:
                ...
                possible_paths = [
                    os.path.join(colcon_source_dir, "riptide_core",
                                 "riptide_descriptions", robot_config_subpath),
                    os.path.join(colcon_source_dir,
                                 "riptide_descriptions", robot_config_subpath),
                ]
                ...
                for path_check in possible_paths:
                    if os.path.exists(path_check):
                        ...
                        self.get_logger().info(
                            f"Discovered source directory, overriding descriptions to use '{self.configPath}'")
                        break

        # set the path to the autocal saved data file
        control_share_dir = get_package_share_directory("riptide_controllers2")

        auto_ff_subpath = os.path.join(
            "config", self.robotName + "_autoff.yaml")
        ...
        if not os.path.exists("/home/ros/colcon_deploy"):
            ...
            self.autoff_config_path = os.path.join(
                control_share_dir, auto_ff_subpath)

        else:
            self.get_logger().info("I think I am running on the orin!")

            self.autoff_config_path = os.path.join(
                ORIN_AUTOTUNE_DIR, self.robotName + "_autoff.yaml")
    ...
    def readConfig(self):
        try:
            ...
            self.get_logger().error(
                f"Cannot open config file at {self.configPath}!")
        ...
        try:
            ...
            self.get_logger().error(
                f"Cannot open config file at {self.autoff_config_path}!")
        except KeyError:
    ...
    def odometryCB(self, msg):
        ...
        try:
            ...
            for i, thursterSufaced in enumerate(self.submergedThrusters):
                pos = self.tfBuffer.lookup_transform(
                    "world", f"{self.tfNamespace}/thruster_{i}", Time())
    ...
    def adjustThrusterWeights(self):
        ...
        if activeThrusterCount <= 6:
            ...
            # remove if PIA
            if self.enabled:
                self.get_logger().error("System has become underactuated. Only:  " +
                                        str(activeThrusterCount) + " thrusters are active. Killing Thrusters!")
                self.enabled = False
        else:
            self.enabled = True
    ...
    def setTeleop(self, request, future):
        ...
        if (request.data == True):
            try:
                ...
                pVal.integer_array_value = [
                    3000000, 3000000, 2000000, 1000000, 1000000, 3000000]
                ...
                self.model_nodes["complete_controller"].set_param_client.schedule_call(
                    request, self.model_nodes["complete_controller"].set_parameters_done_callback)
            ...
            try:
                ...
                pVal.type = ParameterType.PARAMETER_INTEGER_ARRAY
                pVal.integer_array_value = [
                    2000000, 2000000, 2000000, 1000000, 1000000, 2000000]
                ...
                if (type(E) == AttributeError):
                    ...
                    self.get_logger().warn(
                        f"Failed to initialize active control! {E}")
                    future.success = False
    ...
    def doUpdate(self):
        ...
        active_rosnode_names = [node.namespace +
                                "/" + node.name for node in active_rosnodes]

        # remove double leading / which would happen in nodes in the root namespace
        active_rosnode_names = [node_name[1:] if node_name.startswith(
            "//") else node_name for node_name in active_rosnode_names]
        ...
        if (not (self.currentAutoTuneTwist[0] == msg.linear.x and self.currentAutoTuneTwist[1] == msg.linear.y and self.currentAutoTuneTwist[2] == msg.linear.z and
           self.currentAutoTuneTwist[3] == msg.angular.x and self.currentAutoTuneTwist[4] == msg.angular.y and self.currentAutoTuneTwist[5] == msg.angular.z)) or (self.refresh_drag_file):
            ...
            try:
                ...
                self.get_logger().error(
                    f"Cannot open ff auto tune file at: {self.autoff_config_path}")
            except PermissionError:
                self.get_logger().error(
                    f"No Permission to write of autoff file at: {self.autoff_config_path}")
