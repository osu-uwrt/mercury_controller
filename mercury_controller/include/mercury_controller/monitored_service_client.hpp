...
class MonitoredServiceClient {
    ...
    // this is a raw pointer bc only the Node functions are needed so no need to include the full
    // ControllerOverseer
    rclcpp::Node * node;
...
public:
    ...
    */
    bool scheduleCall(Request request, std::function<void(Response)> callback) {
        ...
        RCLCPP_ERROR(
            node->get_logger(), "Reached limit of calls for service %s",
    ...
    void timerCallback() {
        if (waitingForClient) {
            if ((node->get_clock()->now() - startTime).seconds() >= 3) {
                RCLCPP_ERROR(
                    node->get_logger(), "Call to service %s timed out", client->get_service_name());
            ...
            if (waitingRequests.size() > 0) {
                ...
                activeFuture = client->async_send_request(
                    activeClient.request, std::bind(
    ...
    */
    ...
    bool isIdle() { return !waitingForClient && waitingRequests.empty(); }

    // constructor go brrrr
    MonitoredServiceClient(rclcpp::Node * node, std::string serviceName)
        : node(node), waitingForClient(false) {
        client = node->create_client<ServiceT>(serviceName);
        timer = node->create_wall_timer(
            std::chrono::milliseconds(500),
            std::bind(&MonitoredServiceClient<ServiceT>::timerCallback, this));
    }
};
