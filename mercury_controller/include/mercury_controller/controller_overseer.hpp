...
public:
    ...
    */
    ...
    /*
    Set yaml file config paths, right now this is just talos.yaml and talos_autoff.yaml; this will
    change
     */
    void setConfigPath();
    ...
    */
    ...
    /*
    Callback to odometry subscriber, adjusts thruster weights if any thrusters are no longer
    submerged
    */
    void odometryCB(nav_msgs::msg::Odometry::SharedPtr msg);

    /*
    Checks that enough thrusters are working and adjusts weights based on if they are surfaced or if
    in low downdraft mode
    */
    void adjustThrusterWeights();

    /*
    Set teleop service callback that sets controller mask based off yaml file and if teleop mode is
    on.
    */
    void setTeleopCB(
    ...
    /*
    1 second timer callback that lists and sets model parameters if model is active and publishes
    ...
    */
    ...
    std::unique_ptr<tf2_ros::Buffer>
        tfBuffer; // unique ptr because that's how ROS documentation has it
