...
// constructor
SimulinkModelClass::SimulinkModelClass(
    std::shared_ptr<ControllerOverseer> overseerNode, string name)
    : overseer(overseerNode), nodeName(name), modelActive(false), paramsLoaded(false) {
    lastReloadTime = overseer->get_clock()->now();
    reloadParamService = overseerNode->create_service<std_srvs::srv::Trigger>(
        "controller_overseer/update_" + nodeName + "_params",
        std::bind(
            &SimulinkModelClass::reloadParametersCallback, this, std::placeholders::_1,
            std::placeholders::_2));
}
...
void SimulinkModelClass::checkIfActive(const std::vector<string> & activeNodes) {
    ...
    if (amountOfNodes > 1) {
        // should not happen
        RCLCPP_WARN(
            overseer->get_logger(), "Detected %d nodes with %s", amountOfNodes, nodeName.c_str());
    }
    ...
    if (amountOfNodes == 1) {
        ...
        // if the model is not active, set the model as active and make the list/set parameters
        // clients
        if (!modelActive) {
            modelActive = true;
            RCLCPP_INFO(
                overseer->get_logger(), "Found %s as %s!", nodeName.c_str(), fullNodeName.c_str());

            listParamClient = std::make_shared<MonitoredServiceClient<ListParams>>(
                overseer.get(), fullNodeName + "/list_parameters");
            setParamClient = std::make_shared<MonitoredServiceClient<SetParams>>(
                overseer.get(), fullNodeName + "/set_parameters");

            reloadParams();

            // if the model is active and neither client has any actions, list and set the
            // parameters
...
bool SimulinkModelClass::listAndSetModelParameters() {
    // if the model is active and there is a list parameters client, call and list parameters
    // service and then set parameters
    if (modelActive && listParamClient != nullptr) {
        ListParams::Request::SharedPtr req = std::make_shared<ListParams::Request>();

        return listParamClient->scheduleCall(req, [this](ListParams::Response::SharedPtr res) {
            setModelParametersFromListCallback(res);
        });
    }

    RCLCPP_WARN(
        overseer->get_logger(),
        "Cannot list parameters for %s because model is not active or listParamClient is not null",
        nodeName.c_str());
    return false;
}

void SimulinkModelClass::setModelParametersFromListCallback(
    ListParams::Response::SharedPtr response) {
    ...
    if (modelActive) {
        ...
        for (string paramName : paramsToSet) {
            ...
            if (found) {
                ...
                RCLCPP_WARN(
                    overseer->get_logger(), "Not setting %s, parameter not found!",
                    paramName.c_str());
        ...
        setParamClient->scheduleCall(req, [this, req](SetParams::Response::SharedPtr res) {
            this->setParametersDoneCallback(res, req);
        });

    } else {
        RCLCPP_WARN(
            overseer->get_logger(), "Cannot list parameters for %s because model is not active.",
            nodeName.c_str());
    }
}

// Callback for Set Parameters service.
void SimulinkModelClass::setParametersDoneCallback(
    ...
    for (int i = 0; i < sizeof(res->results) / sizeof(res->results[0]); i++) {
        ...
        if (res->results[i].successful &&
            knownParams.find(req->parameters[i].name) == knownParams.end()) {
            knownParams.insert(req->parameters[i].name);
            // if unsuccessful, print
        } else if (!res->results[i].successful) {
            success = false;
            RCLCPP_WARN(
                overseer->get_logger(), "Failed to set parameter: %s for %s: %s",
                req->parameters[i].name.c_str(), nodeName.c_str(), res->results[i].reason.c_str());
        }
    }

    // if all were successful, inform
    if (success) {
        // RCLCPP_INFO(overseer->get_logger(), "Successfully set parameters for %s",
        // nodeName.c_str());
    }
...
void SimulinkModelClass::reloadParametersCallback(
